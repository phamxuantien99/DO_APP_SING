...
interface HomeExternalProps {
  debouncedSearchValue: string;
}

const MyWorkDoneExternal: React.FC<HomeExternalProps> = ({
  ...
  console.log({dataHomeExternalV2});
    ...
    if (isLoadingHomeExternalV2 === false && dataHomeExternalV2) {
      return dataHomeExternalV2.list_component.reduce(
        (acc: Record<string, ListItem[]>, item: ListItem) => {
          // const matchedComponents = item.components.filter((component: any) => {
          //   return (
          //     component.client_supporter === userInfo?.user_info?.user_name
          //   );
          // });

          // Không cần lọc theo username nữa, chỉ lấy toàn bộ components
...
export default MyWorkDoneExternal;
