...
const LastestProjects = ({activeButton}: {activeButton: string}) => {
  const navigation = useNavigation();

  // const {dataDashboard, isLoadingDashboard} = useGetDataDashboard(1, '');
  const {dataHomeInternalV2, isLoadingHomeInternalV2} =
    useGetDataHomeInternalV2(1, '');

  const {dataHomeExternalV2, isLoadingHomeExternalV2} =
    useGetDataHomeExternalV2(1);
  ...
  return activeButton == 'internal' ? (
    <View>
      <Heading text="Lastest Delivery Order" seeAll="see-all-slider-list" />
      ...
      dataHomeInternalV2?.list_component.length === 0 ? (
        ...
        <FlatList
          ...
          data={dataHomeInternalV2?.list_component}
